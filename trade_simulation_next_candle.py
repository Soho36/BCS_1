        ...
        for signal_index, (signal_value, price_level) in enumerate(signal_series):
            ...
            if signal_value == 100 and longs_allowed:
                ...
                for j in range(signal_index, len(filtered_df_original)):
                    next_candle_date = (filtered_df_original.iloc[j+1]['Date']).strftime('%Y-%m-%d')
                    next_candle_time = (filtered_df_original.iloc[j+1]['Time'])
                    next_candle_open = filtered_df_original.iloc[j+1]['Open']
                    next_candle_high = filtered_df_original.iloc[j+1]['High']
                    next_candle_low = filtered_df_original.iloc[j+1]['Low']
                    next_candle_close = filtered_df_original.iloc[j+1]['Close']

                    print('Next candle: ', next_candle_date, next_candle_time, '|',
                          'O', next_candle_open,
                          'H', next_candle_high,
                          'L', next_candle_low,
                          'C', next_candle_close)

                    if next_candle_low <= stop_loss_price and next_candle_high >= take_profit_price:
                        trade_result_both.append(1)
                        print('BOTH')

                    if next_candle_low <= stop_loss_price:
                        ...
                        print(f'□ □ □ Stop Loss hit □ □ □ at {next_candle_date} {next_candle_time}')
                        print()
                    ...
                    elif next_candle_high >= take_profit_price:
                        ...
                        print(f'○ ○ ○ Take profit hit ○ ○ ○ at {next_candle_date} {next_candle_time}')
                        print()
            ...
            # SHORT TRADES LOGIC
                ...
                for j in range(signal_index, len(filtered_df_original)):
                    ...
                    if next_candle_high >= stop_loss_price and next_candle_low <= take_profit_price:
                        trade_result_both.append(1)
                        print('BOTH')

                    if next_candle_high >= stop_loss_price:
                    ...
                    elif next_candle_low <= take_profit_price:
